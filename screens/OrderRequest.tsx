import { Pressable, StyleSheet, TouchableOpacity, Modal, TouchableHighlight, Image, Dimensions } from "react-native";
...
export default function History({ navigation }: Props) {
    const { orderRequest, fetchOrderRequestContext, showLoader2 } = useContext(OrderContext);
    const [modalVisible, setModalVisible] = useState(false);
    const [pressedOrder, setPressedOrder] = useState(null);

    const styleSVG = (item: { status: string }) => {
    ...
    const modalResponse = (item: { status: string }) => {
        if (item?.status == "approved" || item?.status == "accepted") {
                ...
                >
                    ...
                    <View style={styles.modalContainer}>
                        <View style={styles.modalContent}>
                            ...
                            {styleSVG(item) == "#FDC228" && <Image source={require("../assets/images/ProgressSVG.png")} />}
                            {styleSVG(item) == "#074A74" && <SuccessSvg />}
                            <Text style={styles.modalHeading}>
                                Your Order Request <Text style={{ color: styleSVG(item) }}>{modalResponse(item)}</Text>
                            </Text>
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.main}>
                ...
                {showLoader2 ? (
                    <Image source={require("../assets/gifs/loader.gif")} style={styles.image2} />
                ) : (
                    <View
                        style={{
                            backgroundColor: "#fff",
                    ...
                    >
                        {orderRequest?.length > 0 ? (
                                            ...
                                            <View style={styles.order}>
                                                <View style={styles.details}>
                                                    ...
                                                    <View style={styles.title}>
                                                        ...
                                                        <Text style={{ color: "#000", fontSize: 12 }}>Status: {item?.status}</Text>
                                                    </View>
                                                </View>
                                                <Text
                                                    style={{
                            ...
                            >
                                <Image source={require("../assets/images/zerostate2.png")} style={styles.image} />
