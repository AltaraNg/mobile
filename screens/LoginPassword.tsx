import { Pressable, StyleSheet, TextInput, Dimensions, Image } from "react-native";
import * as Device from "expo-device";
import { LinearGradient } from "expo-linear-gradient";
import Header from "../components/Header";
import React, { useState, useEffect } from "react";
import { EyeClose, EyeOpen, SmallCancel, DoubleCheck } from "../assets/svgs/svg";
...
export default function LoginPassword() {
    ...
    const url = `customer/exists/${userPhone}`;
    const auth = useAuth();

    const SetCustomer = (onDefault, NewCustomer, OldCustomer) => {
        return customer == "default" ? onDefault : customer == "new" ? NewCustomer : customer == "old" ? OldCustomer : onDefault;
    };
    const handleLogin = async () => {
        ...
        auth.signInPassword(data.phone_number, data.password, data.device_name, data.login_type, data.customer).then(() => {
...
=======
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.title}>{SetCustomer("Enter phone number", "Welcome 😊", "Welcome Back 😊")}</Text>
            <Text style={styles.simple}>
                {SetCustomer("", "Password must contain a minimum length of 6 characters", "Enter your password to login")}
            </Text>
            <ScrollView>
                        ...
                        <View style={{ backgroundColor: "#EFF5F9" }}>
                            <View style={{ backgroundColor: "#EFF5F9", marginTop: 30 }}>
                                ...
                                <TextInput
                                    onChangeText={(password) => {
                                        setPassword(password);
                                        password == confirmPassword && password.length >= 6 ? setIsDisabled(false) : setIsDisabled(true);
                                    }}
                                    value={Password}
                                    style={styles.input}
                                    secureTextEntry={click ? true : false}
                            ...
                            <View style={{ backgroundColor: "#EFF5F9", marginTop: 30 }}>
                                ...
                                >
                                    {warning && (warning == "notMatch" ? <SmallCancel /> : <DoubleCheck />)}
                                </View>
                                <Pressable
                                    style={{
                                        backgroundColor: "#EFF5F9",
                ...
                <LinearGradient
                    colors={["#074A74", "#089CA4"]}
                    style={[isDisabled ? styles.disabled : styles.buttonContainer, customer !== "default" ? { marginTop: 60 } : {}]}
                    start={{ x: 1, y: 0.5 }}
                    end={{ x: 0, y: 0.5 }}
                >
                    <Pressable style={[styles.button]} onPress={customer == "default" ? checkPhoneNumber : handleLogin} disabled={isDisabled}>
                        {loading ? (
                            <Image source={require("../assets/gifs/loader.gif")} style={styles.image} />
                        ) : (
                            <Text style={styles.buttonText}>{SetCustomer("Continue", "Register", "Login")}</Text>
                        )}
                    </Pressable>
                </LinearGradient>
            </ScrollView>
