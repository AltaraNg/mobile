...
export default function OrderDetails({ navigation, route }: Props) {
    ...
    let repaymentArray = amortization?.map((item: { expected_amount: number }) => {
        return item.expected_amount;
    });
    let totalRepayment = repaymentArray?.reduce((total, item) => {
        return total + item;
    });
    totalPaid =
    ...
    totalDebt = order?.attributes?.down_payment + totalRepayment - totalPaid;
    ...
    const orderStatusChi = (props) => {
        ...
        const expiryDate = new Date(props?.included?.amortizations.find((item) => item.actual_amount == 0)?.expected_payment_date);
    ...
    const nextRepayment = (props: Object) => {
        const nextDate = props?.included?.amortizations.find((item) => item.actual_amount == 0);
    ...
    return (
        <View style={styles.container}>
                                ...
                                <View style={styles.modalContainer}>
                                    <View style={styles.modalContent}>
                                        ...
                                        <Text style={styles.modalText}>
                                            Your loan repayment is <Text style={{ color: "red" }}>overdue</Text> and it has led to additional charges.
                                            Please pay up all your debt completely to avoid further charges
                                        </Text>
                                    </View>
                                </View>
                            </View>
            ...
            <View style={styles.cardContainer}>
                ...
                >
                    ₦{order?.attributes?.product_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                </Text>
            ...
            <View style={styles.amortizationContainer}>
                {lateFees?.length > 0 ? (
                    <View style={styles.toggle}>
                        <LinearGradient
                            colors={!viewLateFee ? ["#074A74", "#089CA4"] : ["#EEEFF0", "#EEEFF0"]}
                            style={!viewLateFee ? styles.buttonContainer : [styles.toggleoff]}
                            start={{ x: 1, y: 0.5 }}
                            end={{ x: 0, y: 0.5 }}
                        >
                            <Pressable style={[styles.button]} onPress={() => setViewLateFee(false)}>
                                <Text
                                    style={
                                        !viewLateFee
                                            ? [styles.buttonText]
                        ...
                        >
                            <Pressable style={[styles.button]} onPress={() => setViewLateFee(true)}>
                                <Text
                                    style={
                                        viewLateFee
                                            ? [styles.buttonText]
                        ...
                        renderItem={({ item }) => (
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        ₦{item.expected_amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                    </Text>
                                </View>
                                <Text
                                    style={{
                                ...
                                >
                                    {checkValid(item.actual_payment_date) ? "Payment date: " : "Due date: "}
                                    <Text
                                        style={{
                                            fontWeight: "bold",
                                            fontSize: 14,
                                    ...
                                    >
                                        {checkValid(item.actual_payment_date) ? item.actual_payment_date : item.expected_payment_date}
                                    </Text>
                                </Text>
                            </View>
                        )}
                    ...
                    >
                            ...
                            renderItem={({ item }) => (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ₦{item.amount_due?.toString()?.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                        </Text>
                                    </View>
                                    <Text
                                        style={{
                                    ...
                                    >
                                        {checkValid(item.date_paid) ? "Payment date: " : "Penalty date: "}
                                        <Text
                                            style={{
                                                fontWeight: "bold",
                                                fontSize: 14,
                        ...
                        <View style={[styles.total, { width: width }]}>
                            ...
                            <LinearGradient
                                ...
                                style={[styles.buttonContainer, { alignItems: "center", justifyContent: "center" }]}
                                start={{ x: 1, y: 0.5 }}
                                end={{ x: 0, y: 0.5 }}
                            >
                                <Text style={[styles.totalText, { color: "white" }]}>
                                    ₦{lateFeeDebt?.toString()?.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                    .00
                                </Text>
                            </LinearGradient>
                        </View>
